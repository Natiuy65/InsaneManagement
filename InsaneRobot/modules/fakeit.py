...
from InsaneRobot import telethn as Galaxy
...
async def _(event):
    ...
    if await is_user_admin(event, event.message.sender_id):
        ...
        if response.status_code == 200:
            with open("InsaneRobot.jpg", "wb") as f:
                f.write(response.content)

        captin = f"Fake Image powered by @always_hungry365."
        fole = "InsaneRobot.jpg"
        await Galaxy.send_file(event.chat_id, fole, caption=captin)
        await event.delete()
        os.system("rm ./Insane.jpg ")
